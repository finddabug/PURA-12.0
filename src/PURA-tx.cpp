...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("PURA Core PURA-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  PURA-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded PURA transaction") + "\n" +
              "  PURA-tx [options] -create [commands]   " + _("Create hex-encoded PURA transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded PURA transaction
